...
class SupportFunction(models.Model):
    ...
    title = models.CharField(SUPPORT_FUNCTION_NAME,
...
class HighLevelStatus(models.Model):
    ...
    title = models.CharField(HIGH_LEVEL_STATUS_NAME,
                             max_length=100, unique=True)
...
class LowLevelStatus(models.Model):
    ...
    title = models.CharField(LOW_LEVEL_STATUS_NAME,
                             max_length=100, unique=True)
...
class LossesForAttack(models.Model):
    ...
    title = models.CharField(LOSSES_FOR_ATTACK_NAME,
                             max_length=100, unique=True)
...
class Project(models.Model):
    ...
    support_function = models.ForeignKey(SupportFunction,
                                         on_delete=models.PROTECT,
    ...
    saving_type = models.ForeignKey(SavingType,
                                    on_delete=models.PROTECT,
                                    related_name='project',
                                    verbose_name=SAVING_TYPE_NAME)
    high_level_status = models.ForeignKey(HighLevelStatus,
                                          on_delete=models.PROTECT,
                                          related_name='project',
                                          verbose_name=HIGH_LEVEL_STATUS_NAME)
    low_level_status = models.ForeignKey(LowLevelStatus,
                                         on_delete=models.PROTECT,
                                         related_name='project',
    ...
    folder_url = models.URLField(
        FOLDER_URL_NAME, blank=True, null=True)

    primary_share = models.FloatField(PRIMARY_SHARE_NAME,
                                      validators=[MinValueValidator(0),
                                                  MaxValueValidator(1)],
                                      default=1)

    secondary_share = models.FloatField(SECONDARY_SHARE_NAME,
                                        validators=[MinValueValidator(0),
                                                    MaxValueValidator(1)],
                                        default=0)

    warehouse_share = models.FloatField(WAREHOUSE_SHARE_NAME,
                                        validators=[MinValueValidator(0),
                                                    MaxValueValidator(1)],
                                        default=0)

    personal_care_share = models.FloatField(PERSONAL_CARE_SHARE_NAME,
                                            validators=[MinValueValidator(0),
                                                        MaxValueValidator(1)],
                                            default=1)

    home_care_share = models.FloatField(HOME_CARE_SHARE_NAME,
                                        validators=[MinValueValidator(0),
                                                    MaxValueValidator(1)],
                                        default=0)

    tea_share = models.FloatField(TEA_SHARE_NAME,
                                  validators=[MinValueValidator(0),
                                              MaxValueValidator(1)],
                                  default=0)

    food_share = models.FloatField(FOOD_SHARE_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)

    ic_share = models.FloatField(IC_SHARE_NAME,
                                 validators=[MinValueValidator(0),
                                             MaxValueValidator(1)],
                                 default=0)

    share_2020 = models.FloatField(SHARE_2020_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)

    share_2021 = models.FloatField(SHARE_2021_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)
    share_2022 = models.FloatField(SHARE_2022_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)
    share_2023 = models.FloatField(SHARE_2023_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=1)
    share_2024 = models.FloatField(SHARE_2024_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)
    share_2025 = models.FloatField(SHARE_2025_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)
    share_2026 = models.FloatField(SHARE_2026_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)
    share_2027 = models.FloatField(SHARE_2027_NAME,
                                   validators=[MinValueValidator(0),
                                               MaxValueValidator(1)],
                                   default=0)
