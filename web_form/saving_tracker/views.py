...
from saving_tracker.models import Project, DATE_FIELDS
from saving_tracker.tables import ProjectTable, DownloadTable
...
def _send_change_email(form):
    ...
    begining = (f'{recepient_name} {recepient_surname}, <br><br>'
    ...
    for field in form.changed_data:
        ...
        changed_value_string = ('<li class="list-group-item">'
                                f'Поле {field}. '
    ...
    def get_context_data(self, **kwargs):
        ...
        for status in project_status:
            ctx[status.lower().replace(' ', '_')] = self.object_list.filter(
                high_level_status__title=status).count()
        ...
        for saving in saving_types:
            ...
            sum_queryset = filtered_queryset.aggregate(
                Sum('saving_potential'))['saving_potential__sum']
